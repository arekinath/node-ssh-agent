...
SSHAgentClient.prototype._request = function (getRequest,
  ...
  socket.on('data', function (data) {
    ...
    len = ctype.ruint32(buf, 'big', 0);

    /* Wait for more data to buffer up */
    if (len > buf.length - 4)
      return;

    data = buf.slice(0, len + 4);
    buf = buf.slice(len + 4);
  ...
  socket.setTimeout(this.timeout, function () {
    if (gotdata || haderror || timedout)
      return;

    timedout = true;
    socket.destroy();
    var e = new TimeoutError('request timed out after: ' + self.timeout +
      ' (had ' + buf.length + ' bytes buffered out of ' + len + ')');
    return callback(e);
  });
